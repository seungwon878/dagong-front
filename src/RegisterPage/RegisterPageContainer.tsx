...
const RegisterPageContainer = ({ bottomButtons, isJoinedMode }: RegisterPageContainerProps) => {
  ...
  const getProduct = async () => {
    ...
    try {
      ...
      const [productDetailResponse, likedItemsResponse] = await Promise.all([
        ...
        fetch(`/member/members/${memberId}/likes`, {
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${authToken}`,
            'Cache-Control': 'no-cache',
  ...
  const handleLike = async () => {
    ...
    try {
        const response = await fetch(`/member/${id}/likes?memberId=${memberId}`, {
            method: method,
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${authToken}`,
  ...
  const handleParticipate = async () => {
    ...
    try {
      const response = await fetch(`/purchases/participate/${id}/${memberId}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${authToken}`,
